 ...
 * Other initialization might also be necessary (PWM for LED's, USART's, USB,
 * etc) and should be performed after the initial call to HWInit
 */
void HWInit() {
  HAL_Init();
  SystemClock_Config();
 ...
 */
uint32_t StmRand(uint32_t Min, uint32_t Max) { return RNG->DR % Max + Min; }
...
void SystemClock_Config(void) {
  ...
  RCC_OscInitStruct.PLL.PLLR =
      2; // NOTE: this line was not supplied by HAL - it simply
  ...
  PeriphClkInitStruct.PeriphClockSelection =
      RCC_PERIPHCLK_USART3 | RCC_PERIPHCLK_CLK48 | RCC_PERIPHCLK_USART2 |
      RCC_PERIPHCLK_UART4;
